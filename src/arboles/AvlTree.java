...
public class AvlTree<T extends Comparable<T>> {
     ...
     * @return int
     */
    private int height(NodeAvl<T> t) {
     ...
     */
    private NodeAvl<T> insert(T element, NodeAvl<T> current) {

        if (current == null) {
            return new NodeAvl<T>(element);
        }
     ...
     */
    private NodeAvl<T> rotateWithLeftChild(NodeAvl<T> k2) {
     ...
     */
    private NodeAvl<T> doubleWithLeftChild(NodeAvl<T> k3) {
     ...
     * @return The node that is being returned is the node that is being rotated.
     */
    private NodeAvl<T> rotateWithRightChild(NodeAvl<T> k2) {
        NodeAvl<T> k1 = k2.right;
        k2.right = k1.left;
        k1.left = k2;
        k2.height = Math.max(height(k2.left), height(k2.right)) + 1;
        k1.height = Math.max(height(k1.left), height(k1.right)) + 1;
     ...
     * @return The node that is being returned is the node that is being rotated.
     */
    private NodeAvl<T> doubleWithRightChild(NodeAvl<T> k3) {
     ...
     */
    private boolean contains(T element, NodeAvl<T> node) {
        if (node == null) {
            return false;
        } else {
     ...
     */
    private void inOrder(NodeAvl<T> root) {
        if (root!=null) {
